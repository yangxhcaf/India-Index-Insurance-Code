...
# Plot temporal and spatial fit  ------------------------------------------
  ...
  # ggplot(data=residual_dist,aes(x=value,group=variable))+ geom_histogram(aes(y = ..density..,fill=variable), alpha=.3, position="identity") + geom_density(aes(colour=variable),size=1.5)  
   ...
   ggplot(data=residual_dist,aes(x=value,group=variable))+  geom_density(aes(colour=variable,fill=variable),size=1.5,alpha=.3)+xlab('Residuals (Wheat Tons/Hectare)')+ylab('Density')+scale_color_manual('',values=c('Pool'=blue,'FE'=green,'Spatial'=purple))+scale_fill_manual('',values=c('Pool'=green,'FE'=blue,'Spatial'=purple))  
...
*Table `r paste(Table_number)`: Root Mean Squared Error (RMSE) for Out-Of-Sample Prediction `r Table_number =Table_number+1 `*
```{r Out-of-Sample Estimation, fig.width=10, fig.height=6, echo=FALSE}
  ...
  DM_PCA_formula_regression = yield_tn_ha ~ 0+factor(district)+rcs(PC1, 4) + rcs(PC2, 4) + rcs(PC3, 4) + rcs(PC4, 4) + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PC11 + PC12 + PC13 + PC14 + PC15 + PC16 + PC17 + PC18 + PC19 + PC20 + PC21
  ...
  RMSE_table = data.frame(`RMSE`=c(RMSE_lm,RMSE_fixed,RMSE_SPLM_FE))
  row.names(RMSE_table) = c('Pool Fit','FE Fit','Spatial Fit')
  panderOptions('table.split.table', Inf)
  panderOptions('big.mark', ",")
  panderOptions('keep.trailing.zeros', F)
  pander(RMSE_table, justify = c('left', 'center'))
