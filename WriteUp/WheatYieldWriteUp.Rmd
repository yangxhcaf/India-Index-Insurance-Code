...
```{r LM for prediction, message=FALSE, warning=FALSE, include=FALSE}
  ...
  PCA_formula_regression =yield_tn_ha~   rcs(PC1,4)+rcs(PC2,4)+rcs(PC3,4)+rcs(PC4,4) 
  
  PCA_formula_regression_dataframe = yield_tn_ha~rcs(PC1,4)+rcs(PC2,4)+rcs(PC3,4)+rcs(PC4,4)+district+years   
...
```{r Choosing Spline min RMSE, echo=FALSE,include=F}

  # search for min RMSE (must have 0 or greater than 3 knots) within sample
  knot = expand.grid(c(0,3,4,5),c(0,3,4,5),c(0,3,4,5),c(0,3,4,5)) # create all unique combinations
  knot$AdjR = NA
  for(i in 1:dim(knot)[1]){
    spline.fm = as.formula(paste( 'yield_tn_ha ~ rcs(PC1,',knot[i,1],') + rcs(PC2, ',knot[i,2],') + rcs(PC3, ',knot[i,3],') + rcs(PC4,',knot[i,4],')'  ))
    lmi = lm(spline.fm,data = pca_pred)
    knot$AdjR[i] = summary(lmi)$adj.r.squared
  
  }
  
  min_RMSE_row = which.max(knot$AdjR)
  PCA_formula_regression = as.formula(paste( 'yield_tn_ha ~ rcs(PC1,',knot[min_RMSE_row,1],') + rcs(PC2, ',knot[min_RMSE_row,2],') + rcs(PC3, ',knot[min_RMSE_row,3],') + rcs(PC4,',knot[min_RMSE_row,4],')'  ))
  PCA_formula_regression_dataframe = as.formula(paste( 'yield_tn_ha ~ rcs(PC1,',knot[min_RMSE_row,1],') + rcs(PC2, ',knot[min_RMSE_row,2],') + rcs(PC3, ',knot[min_RMSE_row,3],') + rcs(PC4,',knot[min_RMSE_row,4],'+district+years )'  ))
...
# search for min RMSE (must have 0 or greater than 3 knots) within sample
  ...
  for(i in 1:dim(knot)[1]){
    spline.fm = as.formula(paste( 'yield_tn_ha ~ rcs(PC1,',knot[i,1],') + rcs(PC2, ',knot[i,2],') + rcs(PC3, ',knot[i,3],') + rcs(PC4,',knot[i,4],')'  ))
    fixed_pca_i <- plm(spline.fm, data=pca_pred, index=c("district", "years"), model="within")
    knot$AdjR[i] =  summary(fixed_pca_i)$r.squared[2] # adj r2 
  }
  ...
  # IMPORTANT: Convert these by hand. rcs(,0) doesn't work with model frame, switch to linear 
  PCA_formula_regression =yield_tn_ha ~ PC1 + rcs(PC2, 4) + rcs(PC3, 5) + PC4
  PCA_formula_regression_dataframe =yield_tn_ha ~ PC1 + rcs(PC2, 4) + rcs(PC3, 5) + PC4+district+years
...
*Table `r paste(Table_number)`: Root Mean Squared Error (RMSE) for Out-Of-Sample Prediction (Tons/Hectare) `r Table_number =Table_number+1 `*
```{r Out-of-Sample Estimation, fig.width=10, fig.height=6, echo=FALSE}
  ...
  load("H:/Projects/India_Index_Insurance/India_Index_Insurance_Code/WriteUp/dataimage.RData")
  ...
  DM_formula_regression = yield_tn_ha ~ 0+factor(district)+ rcs(PC1, 4) + rcs(PC2, 4) + rcs(PC3, 4) + rcs(PC4, 4) 
...
##Spatio-Temporal Crop Prediction
Metrics for wheat yeild prediction at the district level are promising, with within-$R^{2}$ `r paste(range(round(Witin_R2_pool,2),round(Witin_R2_FE,2),round(Witin_R2_splm,2)), collapse=' to ')` and out of sample RMSEs as low as `r paste(min(round(RMSE_lm,2),round(RMSE_fixed,2),round(RMSE_SPLM_FE,2)), collapse=' to ')` metric tons per hectare using only information extracted from NDVI.  The improvements in within and out of sample accuracy from leveraging the spatio-temporal properties of panel data is apparent with predictive accurancy increasing nearly `r round(RMSE_lm/RMSE_SPLM_FE,0)` fold relative to traditional OLS approaches. We can also see the improvements of accuracy obtained through integration of spatially lagged values of yeilds ($W_{ij}Y_{jt}$) are included in the regression. Note however that the inclusion of $Y_{jt}$ on the right side of the equation precludes true forecasting applications because it requires contemporaneous information on yields in neighboring districts. Note however that this particular issue could be at least partially addressed by integrated spatially and lagged values of Y for neighbors *j* ($Y_{jt-1}$), which require no contemporaneous information. 

These differences are clearly exhibited in Figure `r paste(c62)` where the fixed effect and spatial lag models clearly out perform the pooled OLS methodology. This is particularly true during years exhibiting high variance such as 2005 and 2011 where unusually low and high values are prevalent across the sample. This characteristic of panel data reflects the underlying principal of the methodology which is able to model both the time-series properities of a single district, as well as the cross-sectional (spatial) properties of a particular year.   


DISCUSS FIUGRE `r c83`



As for Kharif, this is for the Kharif season preceding the wheat harvest.  It makes sense that a great degree of the variability in the dataset is differences between the Kharif and Rabi seasons. Hence the strong emphasis in PC1.  I included them initially just as an experiment but found that they were reasonably good predictors of rabi season performance. (although note coefficient size in regression pasted below)

I think the explanation is pretty logical. First water availability is a critical component of plant growth and yields. That much is obvious. What isn't generally considered is the fact that water availability from soils is a longer term integrative process (look up CWD), which is some f(rain, temp, solar, soil properties[sandy, loamy, clays]). Lots of rain on sandy soils and hot temps doesn't really help plants as the water runs off or evaporates immediately. (This point was critical in my wildfire work also). Rainfall, esp in rich soils, in the months leading up to a growing season will be important for maintaining soil moisture across the growing season. 
So spillovers from the proceeding season might be missed, especially early season water availability for germination when things are not 'green' yet but water availability is still critical. 

Looking at figure 6 we can see that effects of a failed monsoon in 2004-2005, despite the use of irrigation deficits in soil moisture will be difficult to make up for. 
