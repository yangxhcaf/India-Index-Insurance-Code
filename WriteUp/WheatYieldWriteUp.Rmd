...
```{r Out-of-Sample Estimation, fig.width=10, fig.height=6, echo=FALSE}
  ...
  DM_formula_regression = yield_tn_ha ~ 0+factor(district)+ rcs(PC1, 4) + rcs(PC2, 4) + rcs(PC3, 4) + rcs(PC4, 4) + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PC11 + PC12 + PC13 + PC14 + PC15 + PC16 + PC17 + PC18 + PC19 + PC20 + PC21
  ...
  Xs  = model.matrix( as.formula(form) , pca_pred)                     # get all relevant data
  Bs  = as.vector(c(splm_lag_DM$coefficients , splm_lag_DM$arcoef))    # get coefficients move spatial coef to right
