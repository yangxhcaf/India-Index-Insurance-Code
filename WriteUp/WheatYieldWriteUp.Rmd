...
To avoid problems with multicolinearity between independent variables we apply a principal components analysis transformation (PCA) to a centered and scaled matrix of independent variables $\mathbf{X}$. PCA allows for the replacement of $\mathbf{X}$ with a new matrix whos variables are orthoganal to each other but span the multidimensional space of $\mathbf{X}$ [@GELADI19861]. In this case we generate ```r paste(dim(pca$rotation)[1])``` principal components for inclusion in a random effects panel regression.  

```{r Panel:  PCA summary, echo=FALSE}
 summary(pca) 
  ...
  formula_PCA_dataframe =yield_tn_ha~diff1_yield_tn_ha+rcs(PC1,4)+rcs(PC2,4)+rcs(PC3,4)+rcs(PC4,4)+PC5+PC6+PC7+PC8+PC9+PC10+PC11+PC12+PC13+PC14+PC15+PC16+PC17+PC18+PC19+PC20+PC21 +district+years # add id and year for model.frame
...
(`r paste(F1)`)                
$\ Y_{it}=\alpha+dY_{it-1}+\beta_k \mathbf{X}_{it}+ \mu_{it} +\epsilon_{it}$
 

Where $\ Y_{it}$ is a vector of our dependent variable, district yields in tons per hectare (*yield_tn_ha*), for each district *i* for each year *t*, $\ \alpha$ is an intercept term, $\ dY_{it-1}$ is the first difference of the dependent variable ($\ Y_{it}-Y_{it-1}$), $\ \beta_k$ is a vector of *k* coefficients, $\ \mathbf{X}_{it}$ is a matrix of principal components, $\ \mu_{it}$ is the between entity error term, and $\ \epsilon_{it}$ is the within entity error term. To capture non-linear effects here we apply natural cubic splines to the first `r english(length(grep('rcs', strsplit(as.character(formula_PCA)[3],'+',fixed = T)[[1]])))` principal components[@devlin1986spline] [^1]. Natural cubic splines use cubic terms in the center of the data and restrict the ends to straight lines, preventing distortion at extreme values. When reported splines are noted with rcs(variable_name, number_of_knots) with order noted by ' and ''.

[^1]: The first four principal components in this PCA comprises `r paste(round(summary(pca)$importance[3,4]*100),2)` % of total variance in $\mathbf{X}$
