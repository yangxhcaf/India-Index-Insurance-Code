...
```{r Panel: Spatial autocorrelation on balanced panel, message=FALSE, warning=FALSE, include=FALSE}
  # #distrct boundaries BALANCED
  # districts_lm2 = readOGR('H:/Projects/India_Index_Insurance/Data/Admin Boundaries','PunjabHaryanaDistricts')
  # districts_lm2 = spTransform(districts_lm, CRS('+proj=sinu +a=6371007.181 +b=6371007.181 +units=m'))
  # districts_lm2$NAME_2 = toupper(as.character(districts_lm2$NAME_2)) 
  # districts_lm2 = districts_lm2[districts_lm2$NAME_2 %in% balanced_panel,]
  #   
  # # Create spatial weights  Remove all districts not in yield_ndvi
  # districts_lm2 = districts_lm2[as.character(districts_lm2@data$NAME_2) %in% as.character(unique(yield_ndvi[,'district'])),]
  # districts_polyNB_lm2 = poly2nb(districts_lm2,row.names = row.names(districts_lm2)) # polygon continuity$GEOID10
  # Wneigh_lm2 = nb2mat(districts_polyNB_lm2, style='W')
  # queen2 = nb2listw(districts_polyNB_lm2) 
  # # k nearest weights 
  # centroids2 = coordinates(districts_lm2)
  # knn12 = nb2listw(knn2nb(knearneigh(centroids2, k = 1), row.names = districts_lm2$NAME_2 ))
  # knn22 = nb2listw(knn2nb(knearneigh(centroids2, k = 2), row.names = districts_lm2$NAME_2 ))
  # knn32 = nb2listw(knn2nb(knearneigh(centroids2, k = 3), row.names = districts_lm2$NAME_2 ))
  # knn42 = nb2listw(knn2nb(knearneigh(centroids2, k = 4), row.names = districts_lm2$NAME_2 ))
  # knn52 = nb2listw(knn2nb(knearneigh(centroids2, k = 5), row.names = districts_lm2$NAME_2 ))
  # knn62 = nb2listw(knn2nb(knearneigh(centroids2, k = 6), row.names = districts_lm2$NAME_2 ))
  # 
  # # create balance panel for ols
  ...
  # ols_balanced = pca_pred[pca_pred$district %in% balanced_panel,]
  # ols_balanced = model.frame(PCA_output_formula_dataframe,ols_balanced)
  # 
  # # run for all years seperately
  # holder_mor = data.frame(test = NA, p.value = NA )  
  # for(year in seq(2003,2012)){    
  #   OLS = lm(PCA_output_formula, data=ols_balanced[ols_balanced$years==year,])
  #   OLS_resid = OLS$residuals
  #   moran_queen = moran.test(OLS_resid ,queen2)
  #   moran_knn1  = moran.test(OLS_resid,knn1)
  #   moran_knn2  = moran.test(OLS_resid$residuals,knn2)
  #   moran_knn3  = moran.test(OLS_resid$residuals,knn3)
  #   moran_knn4  = moran.test(OLS_resid$residuals,knn4)
  #   moran_knn5  = moran.test(OLS_resid$residuals,knn5)
