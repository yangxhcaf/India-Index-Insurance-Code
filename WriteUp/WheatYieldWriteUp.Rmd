...
(`r paste(F3)`)                
$\ Y_{it}=\alpha_{i}+\beta_{1}PC(1)_{it-1}+\sum_{k=1}^{K}\beta_{k}PC(k)_{it}+ \mu_{it} +\epsilon_{it}$
 
Where $\ Y_{it}$ is a vector of our dependent variable, district yields in tons per hectare (*yield_tn_ha*) for each district *i* for each year *t*, $\ \alpha_{i}$ are n-1 intercept terms and control for unobserved characteristics of each district *i*, $\ \beta_{1}PC(1)_{it-1}$ is a one-period temporal lag of the first pricipal component, $\ \beta_k$ is a vector of *K* coefficients, $\ X_{it}$ corresponding to the *K* principal components, $\ \mu_{it}$ is the between entity error term, and $\ \epsilon_{it}$ is the within entity error term. To capture non-linear effects here we apply natural cubic splines to the first `r english(length(grep('rcs', strsplit(as.character(PCA_formula_regression)[3],'+',fixed = T)[[1]])))` principal components [@devlin1986spline]. Natural cubic splines use cubic terms in the center of the data and restrict the ends to straight lines, preventing distortion at extreme values. When reported splines are noted with rcs(variable_name, number_of_knots) with order noted by quotes ' and ''.
...
(`r paste(F4)`)                
$\ Y_{it}=...+\lambda\sum_{j=2}^{J}W_{ij}Y_{it}+...+ \gamma_{t} +\epsilon_{it}$

Formula `r Formula_number-1` augments the specification in `r Formula_number-1`. The primary difference is the inclusion of $\lambda\sum_{j=2}^{J}W_{ij}Y_{it}$ where $\lambda$ is called the spatial autoregressive coefficient, $W_{ij}$ is a row standardized weights matrix based for each individual *i* for its neighbors *j* on **polygon continuity**. 
...
```{r Panel: Spatial Panel Estimation, fig.width=10, fig.height=6, echo=FALSE}
 ...
 # SPLM - Spatial lag estimation  -------------------------------------------------
  ...
  Witin_R2 =  1 - (SSR_FULL/SSR_FE)    
  paste('Within R2=',round(Witin_R2,2))
...
# Plot temporal and spatial fit  ------------------------------------------
  ...
  space_time_fit = rbind(model_data_pca,model_pca_splm_lag,model_data_lm)
  ...
  levels(space_time_fit$variable) =c('Actual Yields', 'FE Fit','Spatial Fit','Pool Fit')
  ...
  geom_point(data=space_time_fit[space_time_fit$variable!='Actual Yields',],aes(x= years_id,y=value,colour=variable),alpha=0.6,size=2)+ facet_wrap( ~ district,scales = 'free' )+xlab('Year')+ylab('Wheat Tons/Hectare')+  
...
# Plot temporal and spatial fit  ------------------------------------------
  ...
  # ggplot(data=residual_dist,aes(x=value,group=variable))+ geom_histogram(aes(y = ..density..,fill=variable), alpha=.3, position="identity") + geom_density(aes(colour=variable),size=1.5)  
   ...
   ggplot(data=residual_dist,aes(x=value,group=variable))+  geom_density(aes(colour=variable,fill=variable),size=1.5,alpha=.3)+xlab('Residuals (Wheat Tons/Hectare)')+ylab('Density')  
...
#load("..../dataimage.RData")

  # create wihtin transform variables,  
  demean <- numcolwise(function(x) x - mean(x,na.rm=T))
  means <- numcolwise(function(x) rep(mean(x,na.rm=T),length(unique(pca_pred$years))))
  dm_pca_pred <- ddply(pca_pred, .(district), demean)
  dm_pca_pred$years = pca_pred$years

  # add in neighborhood mean values 
  lagit = function(data,wmatrix){lag.listw(mat2listw(wmatrix),data)} # calc neighborhood mean 
  holder = list() # for demeaned variables
  for(year in unique(dm_pca_pred$years)){holder = c(holder,lagit(dm_pca_pred$yield_tn_ha[dm_pca_pred$years==year],Wneigh))}
  dm_pca_pred$neighbors_mn = unlist(holder)
  holder = list() # for non-demean
  for(year in unique(pca_pred$years)){holder = c(holder,lagit(pca_pred$yield_tn_ha[pca_pred$years==year],Wneigh))}
  pca_pred$neighbors_mn = unlist(holder)
  # calc mean values of all variables by district  
  mn_pca_pred <- ddply(pca_pred, .(district), means) # do this after adding neighborhood mean values
  mn_pca_pred$years = pca_pred$years
  
  ### EXTRACT RELEVANT DATA AND APPLY COEFFICIENTS
  form = as.character(PCA_formula_regression)                             #extract formula used
  form = paste(form[2],form[1],'0+',form[3], " + neighbors_mn",sep=" ")   #append nb density remove constant
  Xs  = model.matrix( as.formula(form) , dm_pca_pred)                     # get all relevant data
  Bs  = as.vector(c(splm_lag$coefficients[-c(1)], splm_lag$spat.coef))    # get coefficients not constant + spatial coef
  XBs = sweep(Xs,MARGIN=2,Bs,`*`)                                         # apply coefficients to demeaned Xs
  mnXBs = model.matrix( as.formula(form) , mn_pca_pred)                   # get mean values for variables
  XBs= XBs+mnXBs                                                          # add means back in after coef applied
  
  P1  = as.data.frame(rowSums(XBs))                                       # non-spatial yhat
  constant = effects.splm(splm_lag)$INTTable[1]
  P1$`rowSums(XBs)` = P1$`rowSums(XBs)` + constant 
  P1$`rowSums(XBs)` = P1$`rowSums(XBs)` + effect
  head(P1)
  
  ###  COMPARE PREDS
  yield = subset(pca_pred, select=c(yield_tn_ha))
  yield$name = row.names(yield)
  P1$name = row.names(P1)
  Preds = merge(P1, yield)
  Preds = cbind(Preds,(splm_lag$model[[1]] - splm_lag$residuals))
